...
**Aug 2026 gotcha:** the catch-all `.html/.swf → /` 301 in the prod static server ran BEFORE static file serving and hijacked a real gated document file. Any extension catch-all must first check `fs.existsSync` in the static dir (fixed in server.js). Symptom: password gate "logs in then shows homepage" (iframe fell back through redirect).
