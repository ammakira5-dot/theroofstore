...
**Second occurrence, different file:** the *production Express server's* own SPA-fallback middleware (for routes that fail to prerender, or during a mid-build state) had the identical bug independently — it read `dist/public/index.html` directly as its meta-injection template instead of the pristine shell backup. Any code path that injects per-route `<title>`/meta into an "index.html template" — not just the build script — must be audited for this same trap. Fix pattern: check for a pristine shell backup file first, fall back to the on-disk index.html only if the backup doesn't exist.
