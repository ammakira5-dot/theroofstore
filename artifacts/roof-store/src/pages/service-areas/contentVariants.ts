...
export const serviceVariants: ((city: string) => { title: string; desc: string }[])[] = [
  (city) => [
    ...
    { title: "Hurricane Protection Coating", desc: `Our Roof Shield system is the world's only TAS-106 Dade County Uplift rated coating — engineered to withstand South Florida hurricane-force winds and keep ${city} homes protected season after season. Clients have the option to add Smart Shield as a top coat over Roof Shield for maximum reflectivity on a dark color choice.` },
    { title: "Roof Repair & Maintenance", desc: `Cracked tiles, failed ridge caps, eave wood damage — we alert you to required roof repair and all problem areas before waterproofing so your ${city} roof is fully sound before any warranty system is applied. Clients are always alerted to any required repairs prior to application. Structural repairs are generally the responsibility of the property owner.` },
  ],
  (city) => [
    ...
    { title: "Hurricane-Rated Coating", desc: `Roof Shield carries the world's only TAS-106 Dade County Uplift rating — built to keep ${city} roofs intact through hurricane season year after year. Clients have the option to add Smart Shield as a top coat for maximum reflectivity on a dark color choice.` },
    { title: "Repair Before Restoration", desc: `Cracked tile, failed ridge caps, damaged eave wood — we alert you to possible structural roof issues on a ${city} roof before any coating or warranty system is applied.` },
  ],
  (city) => [
