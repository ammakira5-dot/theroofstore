...
export const serviceVariants: ((city: string) => { title: string; desc: string }[])[] = [
  (city) => [
    ...
    { title: "Roof Repair & Maintenance", desc: `Cracked tiles, failed ridge caps, eave wood damage — we repair all problem areas before waterproofing so your ${city} roof is fully sound before any warranty system is applied. Clients are always alerted to any required repairs prior to application. Structural repairs are generally the responsibility of the property owner.` },
  ],
  (city) => [
