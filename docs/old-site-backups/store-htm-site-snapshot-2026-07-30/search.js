...
function searchAll(keyword){
	...
	document.write("<center><h3>Showing results <b>"+(display_start+1)+"-"+(displast)+"</b> of <b>"+sites+"</b> for <b>"+keyword+"</b> your search took <b>"+((timeB-timeA)/1000)+"</b> seconds.</h3></center>")
	...
	for (q=display_start; q<q3; q+=1) {
		if (results[q].val>0) {
			...
			if (x1>0 && x2>0) {
				...
				document.write("<table border=0 width=100%><tr><td><span class='xresult'>"+records[rc].description+"</span></td></tr></table><br>")
			}
			q2++
   		}
	}
	...
	if (finds>10) {
		...
		document.write("&nbsp;&nbsp;&nbsp;")
		if (displast<=sites) document.write("<a href='"+paramstring+"&disp="+(displast)+"'>Next page</a><br><br>")
...
function AddBody() {
	...
	templateBody()
	document.write("<center><form name='searchform' method='post' action='javascript:doSearch()'><table border='0' cellspacing='0' cellpadding='0'><tr><td><input size='10' name='searchwords'></td><td><a href='javascript:doSearch()'>"+ButtonCode+"</a></td></tr></table></form></center>")
