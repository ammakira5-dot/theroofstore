...
- **🔴 FIRST THING TOMORROW — GA4 Install (15 min)** — Go to analytics.google.com → create a GA4 property for theroofstore.net → copy the Measurement ID (format: G-XXXXXXXXXX) → give to agent. Every month without GA4 is a month of traffic damage evidence lost permanently. Agent installs in 15 minutes once ID is provided.

- **🔴 FIRST THING TOMORROW — GSC → Google Sheets export (damages evidence)** — Open Google Search Console → Performance → date range: max 16 months → filter query: "the roof store" → Export → Google Sheets → share sheet with agent. Agent will then build a month-by-month impressions/clicks erosion chart as Exhibit K-2 — real platform-native data, no estimates, no paid tools needed.
...
- **🔴 P1 — GBP optimization (owner action — business.google.com)** — Google Business Profile for The Roof Store needs full optimization: (1) Upload 10+ job photos (before/after tile roofs, crew, product pails); (2) Complete services list — Tile Roof Coating, Flat Roof Waterproofing, Roof Painting, Commercial Roof Coating, Hurricane Roof Prep, etc.; (3) Business description (750 chars) — hit keywords: "roof coating," "tile roof," "Broward County," "South Florida," "TAS-106," "Storm Shield"; (4) Seed Q&A — post 5–6 common questions + answers yourself; (5) Confirm hours are accurate; (6) Google Posts — share the 3 new blog articles as GBP posts; (7) Reviews — text/email past customers, target 50+ in 60 days. Agent can draft business description + Q&A seed questions on request.
