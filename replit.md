...
- **Factory price sheet auto-response — BUILT, blocked by Porkbun DNS** — The full factory price sheet email (RP1/RP2/RP3 pricing tables by roof type and pitch, notes, $150 inspection note, CTA) is fully built in `artifacts/api-server/src/lib/email.ts` → `buildAutoResponseHtml()`. The RoofProtect product presentation PDF is already attached. Everything works — it currently sends to ammakira5@gmail.com only. To unlock delivery to all client emails: complete Resend domain verification (see priority item below).
