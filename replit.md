...
- **Schema fix bundle (ON HOLD — user said wait, July 28)** — (1) /products page: de-duplicate 6→3 Product schemas in Products.tsx (bare CollectionPage list items missing image/description are the "invalid" merchant listings in GSC; replace with plain URL refs). (2) Add `shippingDetails` + `hasMerchantReturnPolicy` to all product Offers — NEED owner's shipping + return policy first. (3) Optional `certificationRating` on TAS-106 cert (may not apply — pass/fail). (4) VideoObject `uploadDate` on /videos is placeholder "2024-01-01" without timezone — replace with each video's real YouTube publish date in full ISO 8601 w/ timezone (GSC non-critical, both videos otherwise valid). Product detail pages + both video schemas confirmed valid & rich-result eligible July 28.
- **Duplicate URL fix (AWAITING APPROVAL, July 28 audit)** — /roof-services/[county] and /service-areas/[county] have identical titles + both in sitemap = confirmed cannibalization (4 counties). Recommended: 301 /roof-services/[county] → /service-areas/[county]. Audit done, fix not applied.
