...
- **windproofroofsystems.com — 4 content fixes pending** — FTP host + user confirmed: `ftp.windproofroofsystems.com` (or `shrimp.exacthosting.com`), user: `Fullstack2@windproofroofsystems.com`. **Blocked: password in `WINDPROOF_FTP_PASSWORD` secret is wrong — 530 auth error on both hosts.** User needs to reset the FTP password in cPanel (exacthosting.com) and update the secret. Then agent can do all 4: (1) Remove "A+ BBB Rated since 1994" claim; (2) Fix tel href `tel:954-900-1973` → correct href for 954-743-0667; (3) Replace verbatim FungalShield/SmartShield/RoofShield copy with tile-restoration-specific paragraph; (4) Remove shared testimonials (Don Godshall, Ana Arias, Arlene Kalb). Once done: re-enable partner link in `PartnerNetwork.tsx` + restore "owns" WebSite schema in `Home.tsx` + `index.html`.

- **roofrestorationsandwaterproofinginc.com — 3 fixes pending** — Password secret `RRSW_FTP_PASSWORD` exists but **FTP host + username unknown** (login failed). Need: FTP host + username. Then agent can do all 3: (1) Fix tel link `tel:954-900-1973` → `tel:8778245823`; (2) Remove "A+ BBB Rated since 1994" claim; (3) Rewrite FungalShield/SmartShield/RoofShield section (currently verbatim copy from theroofstore.net).
