...
- **windproofroofsystems.com — 4 content fixes pending** — Password secret `WINDPROOF_FTP_PASSWORD` exists but **FTP host + username unknown** (login failed). Need: FTP host (e.g. `ftp.windproofroofsystems.com` or hosting server), username. Then agent can do all 4: (1) Remove "A+ BBB Rated since 1994" claim; (2) Fix tel href `tel:954-900-1973` → correct href for 954-743-0667; (3) Replace verbatim FungalShield/SmartShield/RoofShield copy with tile-restoration-specific paragraph; (4) Remove shared testimonials (Don Godshall, Ana Arias, Arlene Kalb). Once done: re-enable partner link in `PartnerNetwork.tsx` + restore "owns" WebSite schema in `Home.tsx` + `index.html`.
